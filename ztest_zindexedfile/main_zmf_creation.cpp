...
#include <zremote/zbasenet/zsslclientsocket.h>

#include <zremote/zbasenet/zconsolecommand.h>


#include <zflow/zflowcommon.h>
...
#ifdef __COMMENT__
 void
 printIndexKey_0(ZSMasterFile &ZPicture)
...
//              wIndexContent.ZMFaddress,
 ...
 void
 printIndexKey_1(ZSMasterFile &ZPicture,const zrank_type pIndexRank)
...
public:
                           // Caller Mode
    Docid_struct        Documentid;     //R    Documentid is the unique document identifier generated by DocumentServer : a long
    descString          Desc ;          //RW
    codeString          DocMetaType;    //RW
    uint32_t            Storage;        //RW   see Storage_type enum definition (ztypes.h) write once when physical document is created
    descString          MimeName;       //R
    ...
    identityString  OwnerOrigin;    //R
    identityString  OwnerCurrent;   //RW
    uint32_t        AccessRights;   //RW
    ...
    UserId_type     OwnerId;         //     User Identification number to whom belongs the Document
...
*/
    ...
    ZMetaDic wMetaDic;                      // empty meta dictionary
...
//fieldDesc_struct wField;
    ...
    long wDocid;
    wSt=wMetaDic.addField_A(wDocid,(const utf8_t*)"Documentid");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.Desc,(const utf8_t*)"Desc");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wSt=wMetaDic.addField_A(wDoc.DocMetaType,(const utf8_t*)"DocMetaType");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wSt=wMetaDic.addField_A(wDoc.Storage,(const utf8_t*)"Storage");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wSt=wMetaDic.addField_A(wDoc.MimeName,(const utf8_t*)"MimeName");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wSt=wMetaDic.addField_A(wDoc.URI,(const utf8_t*)"URI");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.Size,(const utf8_t*)"Size");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.Registrated,(const utf8_t*)"Registrated");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.Created,(const utf8_t*)"Created");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.LastModified,(const utf8_t*)"LastModified");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
    wSt=wMetaDic.addField_A(wDoc.OwnerOrigin,(const utf8_t*)"OwnerOrigin");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.OwnerCurrent,(const utf8_t*)"OwnerCurrent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.AccessRights,(const utf8_t*)"AccessRights");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wMetaDic.addField_A(wDoc.CheckSum,(const utf8_t*)"CheckSum");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wSt=wMetaDic.addField_A(wUtf8Content,(const utf8_t*)"DocumentContent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    wMetaDic.print(stdout);

    ZSKeyDictionary wKDic(&wMetaDic);

    wKDic.addFieldToZDicByName((const utf8_t*)"Documentid");

    setZMFStatistics(false);        // default value

    ZException.setAbortOnSeverity(Severity_Severe);

    ZSMasterFile ZPicture;

    addVerbose(ZVB_ZMF);

    printf ("**Main** zcreate \n");

    wSt=ZPicture.zcreate(&wMetaDic,"/home/gerard/testindexedv3.zmf",100000,false,false );
    if (wSt!=ZS_SUCCESS)
                ZException.exit_abort();
    wSt=ZPicture.zcreateIndex(wKDic,                // the key Dictionary that just has been set
                              (utf8_t*)"First Index",        // user defined name
    ...
    wSt=ZPicture.zopen("/home/gerard/testindexedv3.zmf",ZRF_All);

// populate the file


    wDoc.URI = (const utf8_t*)"/home/gerard/refman.rtf";
//    wDoc.URI.loadContent(wZDBContent);


    wDoc.URI.loadUtf8(wUtf8Content);


    wDoc.Size= wUtf8Content.getByteSize();
    wDoc.OwnerOrigin="gerard";

    wDoc.CheckSum=wUtf8Content.getcheckSum();


    wDoc.Documentid.getNext();
    wDoc.Desc = (const utf8_t*)"First document text";
    ...
    ZRecord* wRecord=ZPicture.generateZRecord();

    wRecord->init();

    wDoc.Documentid=123456789;
    wDoc.LastModified.getCurrentDateTime();
    wSt=wRecord->setFieldValuebyName(wDoc.Documentid.id,"Documentid");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName(wDoc.Desc,"Desc");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName(wDoc.LastModified,"LastModified");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName(wDoc.URI,"URI");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
    wSt=wRecord->setFieldValuebyName(wUtf8Content,"DocumentContent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName(wDoc.Size,"Size");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName(wDoc.MimeName,"MimeName");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
    wSt=wRecord->setFieldValuebyName(wDoc.CheckSum,"CheckSum");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);

    printf ("**Main** Add record\n");

    _SET_ZMFVERBOSE_; _SET_ZRFVERBOSE_ ;
...
zxmlDoc wXMLDoc;
    ...
    wSt=wXMLDoc.createBlankRootElement(wRoot,"ZSMasterFile",nullptr);
   _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
    ...
    ZPicture.zopen("/home/gerard/testindexedv3.zmf",ZRF_All);

    ZRecord* wRecordIn=ZPicture.generateZRecord();
    wSt=ZPicture.zget(wRecordIn,0L);

    wRecordIn->RecordMap(stdout);

    wSt=ZPicture.zprintIndexFieldValues(0L,0L,true,true); // printf index fields for index OL rank 0L (first raw of first index

    long wId ;
    wSt=wRecord->getFieldValuebyRank_T<long>(wId,0L);
    utf8VaryingString wMime;
    wSt=wRecord->getFieldValuebyName(wMime,"MimeName");
    checkSum wCheckSum;
    wSt=wRecord->getFieldValuebyName(wCheckSum,"CheckSum");
    ZDateFull wDate;
    wSt=wRecord->getFieldValuebyName(wDate,"Created");
    wSt=wRecord->getFieldValuebyName(wUtf8Content,"DocumentContent");

    utf8VaryingString wWContent;
    wSt=wRecord->getFieldValuebyName(wWContent,"DocumentContent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
