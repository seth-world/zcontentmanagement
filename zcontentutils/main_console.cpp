...
//fieldDesc_struct wField;
    ...
    ZSKeyDictionary wK0Dic("Primary key - by document id",&wMasterDic);

    wSt=wK0Dic.addFieldToZKeyByName("Documentid");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)

    ZSKeyDictionary wK1Dic("Key 1 - by description",&wMasterDic);

    wSt=wK1Dic.addFieldToZKeyByName("Desc");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)


    long wRank=wMasterDic.addKey(&wK0Dic,"Primary key - by document id");
    _ASSERT_ZEXCEPTION_(wRank < 0)

    wRank=wMasterDic.addKey(&wK1Dic,"Key 1 - by description");
    _ASSERT_ZEXCEPTION_(wRank < 0)
    ...
    wSt=ZPicture.zcreate(&wMasterDic,__TEST_FILE__,100000,false,false );
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    ...
    wSt=ZPicture.zcreateIndex(&wK0Dic,                // the key Dictionary that just has been set
    ...
    wSt=ZPicture.zcreateIndex(&wK1Dic,                // the key Dictionary that just has been set
...
// populate the file
 ...
 //   wDoc.URI = "/home/gerard/refman.rtf";
    ...
    wDoc.DocSize= wUtf8Content.getByteSize();

//    wDoc.OwnerOrigin= ZSystemUser::getUserId();  // not correct : use authorization manager
    ...
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.Documentid.id)>(wDoc.Documentid.id,"Documentid");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.Desc)>(wDoc.Desc,"Desc");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.LastModified)>(wDoc.LastModified,"LastModified");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.URI)>(wDoc.URI,"URI");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wUtf8Content)>(wUtf8Content,"DocumentContent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.DocSize)>(wDoc.DocSize,"DocSize");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.MimeName)>(wDoc.MimeName,"MimeName");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
    wSt=wRecord->setFieldValuebyName_T<decltype(wDoc.CheckSum)>(wDoc.CheckSum,"CheckSum");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
...
zxmlDoc wXMLDoc;
   ...
   _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS);
    ...
    wSt=wRecord->getFieldValuebyName_T<decltype(wMime)>(wMime,"MimeName");
    checkSum wCheckSum;
    wSt=wRecord->getFieldValuebyName_T<decltype(wCheckSum)>(wCheckSum,"CheckSum");
    ZDateFull wDate;
    wSt=wRecord->getFieldValuebyName_T<decltype(wDate)>(wDate,"Created");
    wSt=wRecord->getFieldValuebyName_T<decltype(wUtf8Content)>(wUtf8Content,"DocumentContent");

    utf8VaryingString wWContent;
    wSt=wRecord->getFieldValuebyName_T<decltype(wWContent)>(wWContent,"DocumentContent");
    _ASSERT_ZEXCEPTION_(wSt!=ZS_SUCCESS)
